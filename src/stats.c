...
void report_time(void) {
  ...
  for (i=0; i<NR_FUNCS; i++) {
    ...
    if (count_sum > 0) {
      //printf("%s: %e\n", func_name[i], (double)time_sum.tv_sec + (double)time_sum.tv_nsec / (double)1000000000);
      //total = (double)time_sum.tv_sec + (double)time_sum.tv_nsec / BILLION;
      total = (double)time_sum.tv_nsec;
      average = total / count_sum;
      printf("[%s] total %lf nsec, %d calls, average %lf nsec\n", func_name[i], total, count_sum, average);
    }
  }
}
